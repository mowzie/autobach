...
def parse_hymn(filename):
    ...
    new_name = f"{hymn_number} {title}"
    
    return new_name


def parse_liturgy(filename):
    # Remove the extension
    name, _ = os.path.splitext(filename)
...
def convert_type_0_to_type_1(input_file, output_file, parsed_filename):
    ...
    for msg in mid.tracks[0]:
        ...
        if isinstance(msg, MetaMessage) and msg.type == 'smpte_offset':
            track_name_message = MetaMessage('track_name', name=f"{parsed_filename}", time=0)
            composer_track.append(track_name_message)

        elif isinstance(msg, MetaMessage) or msg.type == 'sysex':
...
if __name__ == "__main__":
    ...
    parsed_filename = parse_hymn(filename_with_extension)
